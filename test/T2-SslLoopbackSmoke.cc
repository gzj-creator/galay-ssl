...
Coroutine runServer(IOScheduler* scheduler, SslContext* ctx, SmokeState* state)
{
    (void)scheduler;
...
Coroutine runClient(SslContext* ctx, SmokeState* state)
{
...
int main()
{
    ...
    expect(scheduler.spawn(runServer(&scheduler, &serverCtx, &state)), "spawn server failed");
    waitFor(state.serverReady, "server did not become ready");
    expect(scheduler.spawn(runClient(&clientCtx, &state)), "spawn client failed");
