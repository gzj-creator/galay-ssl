...
while (!handshake_completed) {
    ...
    case Success:
        if (engine.pendingEncryptedOutput() > 0) {
            if (auto __await_result = co_await sendEncryptedData(); !__await_result) {
                // 错误处理：记录日志、重试或提前返回
            }
        }
    ...
    case WantRead:
        ...
        if (auto __await_result = co_await recvEncryptedData(); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
...
```cpp
// 正确
if (auto __await_result = co_await socket.send(data1, len1); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await socket.send(data2, len2); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
```cpp
// 挂起当前协程
if (auto __await_result = co_await awaitable; !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
