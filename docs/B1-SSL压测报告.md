...
### 单连接性能
- 单连接场景下性能稳定，错误率为0%
- QPS随请求数增加而提升，说明SSL会话复用生效
- 单连接最高可达 43,478 QPS

### 多连接并发性能
- 10连接并发时达到最高QPS (85,524)
- 随着连接数增加，错误率略有上升
- 高并发场景下存在少量连接竞争
...
|------|---------|-------|---------|-----|-------------|
| galay-ssl | 100,000 | 0 | 803.8 (773 / 851) | 124,554 (117,509 / 129,366) | 11.17 (10.53 / 11.60) |
...
| 框架 | 完成请求 | 错误数 | 耗时(ms) | QPS | 吞吐量(MB/s) |
|------|---------|-------|---------|-----|-------------|
| galay-ssl | 2,000 | 0 | 147.0 (142 / 151) | 13,612 (13,245 / 14,084) | 1,701.47 (1,655.63 / 1,760.56) |
...
|------|-------------------|-----------------|--------|
| **QPS** | 320,799 | 124,554 | 38.8% |
| **吞吐量** | 156.64 MB/s | 11.17 MB/s | 7.1% |
| **错误率** | 0% | < 1% | - |
