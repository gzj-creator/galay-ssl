...
- **构建口径**: 统一使用 `Release + LTO`（`cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DENABLE_LTO=ON`）
...
- **最高QPS**: 162,602（200连接，47B 小包，5轮最大值）
- **最高吞吐量**: 2,000.00 MB/s（10连接，64KiB 大包，5轮最大值）
- **小包平均QPS**: 162,234（5轮平均）
- **大包平均吞吐量**: 1,981.10 MB/s（5轮平均）
...
### 多连接并发性能
- 小包场景（200连接，47B）5 轮均无错误，QPS 区间 161,551 ~ 162,602
- 大包场景（10连接，64KiB）5 轮均无错误，吞吐区间 1,953.12 ~ 2,000.00 MB/s
- 相比上一轮 5 轮统计，小包平均 QPS 由 129,018 提升到 162,234（+25.7%）
...
|------|---------|-------|---------|-----|-------------|
| galay-ssl | 100,000 | 0 | 616.4 (615 / 619) | 162,234 (161,551 / 162,602) | 14.54 (14.48 / 14.58) |
...
| 框架 | 完成请求 | 错误数 | 耗时(ms) | QPS | 吞吐量(MB/s) |
|------|---------|-------|---------|-----|-------------|
| galay-ssl | 2,000 | 0 | 126.2 (125 / 128) | 15,849 (15,625 / 16,000) | 1,981.10 (1,953.12 / 2,000.00) |
...
|------|-------------------|-----------------|--------|
| **QPS** | 320,799 | 162,234 | 50.6% |
| **吞吐量** | 156.64 MB/s | 14.54 MB/s | 9.3% |
| **错误率** | 0% | < 1% | - |
