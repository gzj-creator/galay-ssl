...
// 第一次连接
{
    ...
    if (auto __await_result = co_await socket.connect(host); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
    // ... 握手 ...
...
for (const auto& buf : buffers) {
    if (auto __await_result = co_await socket.send(buf.data(), buf.size()); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    }
}
...
if (auto __await_result = co_await socket.send(merged.data(), merged.size()); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
if (auto __await_result = co_await socket.send(reinterpret_cast<const char*>(data.data()), data.size()); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
Coroutine stressTest(SslContext* ctx, size_t num_connections) {
    ...
    for (size_t i = 0; i < num_connections; ++i) {
        tasks.push_back([ctx]() -> Coroutine {
            SslSocket socket(ctx);
            if (auto __await_result = co_await socket.connect(Host(IPType::IPV4, "127.0.0.1", 8443)); !__await_result) {
                // 错误处理：记录日志、重试或提前返回
            }

            while (!socket.isHandshakeCompleted()) {
                if (auto __await_result = co_await socket.handshake(); !__await_result) {
                    // 错误处理：记录日志、重试或提前返回
                }
            }

            if (auto __await_result = co_await socket.send("test", 4); !__await_result) {
                // 错误处理：记录日志、重试或提前返回
            }

            char buf[1024];
            if (auto __await_result = co_await socket.recv(buf, sizeof(buf)); !__await_result) {
                // 错误处理：记录日志、重试或提前返回
            }
