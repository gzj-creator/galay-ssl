...
if (auto __await_result = co_await socket.send(batch.data(), batch.size()); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
while (retries-- > 0) {
    ...
    if (auto __await_result = co_await std::chrono::milliseconds(1000); !__await_result) {
        // 错误处理：记录日志、重试或提前返回
    } // 等待后重试
}
```
...
public:
    ...
    Coroutine acquire() {
        ...
        // 等待可用连接
        if (auto __await_result = co_await waitForRelease(); !__await_result) {
            // 错误处理：记录日志、重试或提前返回
        }
...
// 2. 等待现有连接处理完成
if (auto __await_result = co_await waitForActiveConnections(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
